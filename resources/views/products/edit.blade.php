...
<form class="text-gray-700 body-font py-24 px-24 bg-teal-200 rounded md:mt-20"
    action="{{ route('product.update', ['product' => $product]) }}" method="POST" enctype="multipart/form-data">
    @csrf
    @method('PATCH')
    <div class="flex">
        <div class="md:w-1/4">
    ...
    <div class="flex">
        <div class="w-1/2">
            ...
            <div class="md:mr-5">
                <select
                    ...
                    <option value="Sale" {{ $product->badge == 'Sale' ? 'selected' : '' }}>New</option>
        ...
        <div class="w-1/2">
            <div class="md:ml-5">
                <textarea
                    class="w-full bg-white rounded border border-gray-400 focus:outline-none h-24 focus:border-teal-500 text-base px-4 py-2 mb-4 resize-none" name="short_description">{{ $product->short_description }}</textarea>
            </div>
            <div class="md:ml-5 md:mt-2">
                <textarea
                    class="w-full bg-white rounded border border-gray-400 focus:outline-none h-32 focus:border-teal-500 text-base px-4 py-2 mb-4 resize-none" name="long_description">{{ $product->long_description }}</textarea>
            </div>
            <div class="md:ml-5 md:mt-3">
                <div class="">
                    <img class="object-contain h-48 w-full" src="{{ asset('storage/images/'.$product->image) }}" alt="">
                </div>
